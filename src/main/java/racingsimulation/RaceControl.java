...
public class RaceControl extends AbstractBehavior<RaceControl.Command> {
  ...
  public Receive<RaceControl.Command> createReceive() {
    return newReceiveBuilder()
              ...
              return Behaviors.withTimers(
                  timer -> {
                    timer.startTimerAtFixedRate(
                        timer_key, new GetPositionsCommand(), Duration.ofSeconds(1));
                    return Behaviors.same();
                  });
            })
        .onMessage(
            GetPositionsCommand.class,
            command -> {
              currentPositions
                  .keySet()
        ...
        .onMessage(
            RacerUpdateCommand.class,
            command -> {
              currentPositions.put(command.from, command.position);
              return Behaviors.same();
            })
        .onMessage(
            RacerCompletedCommand.class,
            command -> {
    ...
    return newReceiveBuilder()
        .onMessage(
            ...
            command -> {
              ...
              finishingTimes.forEach(
                  (racer, end) ->
                      System.out.println(
                          racer.path().name() + ": " + (end - start) / 1000.0 + " seconds"));
              return Behaviors.withTimers(
                  timers -> {
                    timers.cancelAll();
